...
+function($, window, document, Math) {
	...
	var desktopPos = {
		x : 40,
		y : 0
	}, fullscreenMode = false, windowIdSeed = 0, windowIdTeamplate = 'wid-{0}', windowZIndexSeed = 100, defaultWindowPos = {
		x : 110,
		y : 20
	}, entries = null, desktop = null, superadmin = false, windows = null, theLoadingFrame = null;
	 ...
	 * 
	 * @param object options @retrun void
	 */
	...
	function refreshEntries(entriesOptions, reset) {
		...
		if (reset) {
			$.each(entries, function(idx, et) {
				if (et.id != 'allapps' && et.id != 'superadmin'
						&& et.id != 'dashboard')
					et[DEL] = true;
			});
		}
	 ...
	 * The entry object
	 * 
	 * Create a entry object, example: 'var et = new entry({});' A entry object
	 * stored the entry options and has a method to create a window object
	 * 
	 * @param object
	 *            options
	 */
	...
	entry.prototype.resetPosSize = function() {
		/* Init size setting */
		if (typeof this.size === 'string' && this.size.indexOf('}') > 0) // convert
																			// size
																			// data
																			// like
		...
		if (this.size == 'default') {
			...
			if (this.cssclass.indexOf(' window-movable') < 0)
				this.cssclass += ' window-movable';
		} else if (this.size.width !== undefined
			...
			if (this.cssclass.indexOf(' window-max') < 0)
				this.cssclass += ' window-max';
		}

		/* Init position setting */
		if (this.position == 'center') {
			this.left = Math.max(desktop.x, desktop.x
					+ (desktop.width - this.width) / 2);
			this.top = Math.max(desktop.y, desktop.y
					+ (desktop.height - this.height) / 2);
		} else if (this.position.x !== undefined
				&& this.position.y !== undefined) {
			this.left = Math.max(desktop.x, this.position.x);
			this.top = Math.max(desktop.y, this.position.y);
		} else {
			defaultWindowPos = {
				x : defaultWindowPos.x + settings.defaultWinPosOffset,
				y : defaultWindowPos.y + settings.defaultWinPosOffset
			};
		...
		if (this.display != 'fixed' && this.size != 'max'
				&& this.cssclass.indexOf(' window-movable') < 0)
			this.cssclass += ' window-movable';
	};
	...
	entry.prototype.toTaskBarShortcutHtml = function() {
		if (this.display != 'modal')
			return settings.taskBarShortcutTplt.format(this);
	};
	...
	entry.prototype.createWindow = function() {
		if (this.display == 'modal')
			$('#modalContainer').append(this.toWindowHtml());
		else
			$('#deskContainer').append(this.toWindowHtml());
	...
	entry.prototype.init = function(options, ignoreDefault) {
		if (!desktop) {
			...
			desktop = {
				x : desktopPos.x,
				y : desktopPos.y,
				width : w.width() - desktopPos.x,
				height : w.height() - desktopPos.y
			};
		}

		/* extend options from params */
		$.extend(this, ignoreDefault ? null : this.getDefaults(options.id),
				this, options);
		...
		if (this.icon.indexOf('icon-') == 0)
			this.iconhtml = '<i class="icon ' + this.icon + '"></i>';
		else if (this.icon.length > 0)
			this.iconhtml = '<img src="' + this.icon + '" alt="" />';
		else {
			var name = this.abbr || this.name;
			var nameL = name.length;
			this.iconhtml = '<i class="icon icon-default" style="background-color: hsl('
					+ (this.id * 47 % 360)
					+ ', 100%, 40%)"><span>'
					+ (nameL > 0 ? name.slice(0, 1).toUpperCase() : '')
					+ '</span><span class="text-extra">'
					+ (nameL > 1 ? name.slice(1, 2).toUpperCase() : '')
					+ '</span></i>';
		}
	...
	desktopManager.prototype.init = function() {
		...
		this.updateBrowserUrl(indexUrl, true, 'index', {
			tag : 'index'
		});
	}
	...
	desktopManager.prototype.bindEvents = function() {
		$(window).resize($.proxy(function() // handle varables when window size
											// changed
		{
			/* refresh desktop size */
			this.size = {
				width : this.$.width() - this.x,
				height : this.$.height() - this.y - settings.bottomBarHeight
			};
	...
	desktopManager.prototype.refreshMenuSize = function() {
		var $menu = this.$menu;
		var $icons = $menu.children('.bar-menu:not(#moreOptionMenu)').find(
				'li:not(#s-menu-allapps)');
		...
		if (moreOption) {
			...
			if ($btn.data('icons-count') != (iconsCount - start)) {
				var $moreIcons = $icons.removeClass('option').slice(start)
						.addClass('option');
				...
				$moreIcons.children('.app-btn').attr('data-btn-type', 'list')
						.attr('data-placement', 'bottom').attr(
								'data-tip-class', '');

				$btn.addClass('active').data('icons-count', $moreIcons.length)
						.data('icons', $moreIcons).attr(
								'data-original-title',
								settings.moreOptionTip
										.format($moreIcons.length));
				setTimeout(function() {
	...
	desktopManager.prototype.updateBrowserUrl = function(url, isForcePush,
			title, state) {
		url = url || indexUrl;
		state = state || {};
		try {
	...
	windowsManager.prototype.query = function(q) {
		return this.opens[q]
				|| this.opens[settings.windowidstrTemplate.format(q)]
				|| this.activedWindow;
	};
	...
	windowsManager.prototype.activeLastWindow = function(toUnActive) {
		if (toUnActive)
			this.unActiveWindow(toUnActive);
	...
	windowsManager.prototype.activeWindow = function(win) {
		if (this.shows.length)
			this.opens[settings.windowidstrTemplate
					.format(this.shows[this.shows.length - 1])].$
	...
	windowsManager.prototype.bindEvents = function() {
		$('#desktop')
				.on('click', '.window', function() // active by click a non
													// actived window
				{
				...
				.on(
						'mousedown',
						'.movable,.window-movable .window-head',
						function(e) // make the window movable with class
									// '.movable' or '.window-movable'
						{
							var win = $(this).closest(
									'.window:not(.window-max)');
							if (win.length < 1)
								return;
							...
							win.data('mouseOffset', {
								x : e.pageX - mwPos.left,
								y : e.pageY - mwPos.top
							}).addClass('window-moving');
							$(document).bind('mousemove', mouseMove).bind(
									'mouseup', mouseUp)
							e.preventDefault();

							function mouseUp() {
								$('.window.window-moving').removeClass(
										'window-moving');
							...
							function mouseMove(event) {
								if (windows.movingWindow
										&& windows.movingWindow
												.hasClass('window-moving')) {
									var offset = windows.movingWindow.$
											.data('mouseOffset');
				...
				// listen the keydown events
				.on(
						'click',
						'.max-win',
						function(event) // max-win
						{
							windows.opens[$(this).closest('.window').attr('id')]
									.toggleSize();
							stopEvent(event);
						})
				.on(
						'dblclick',
						'.window-head',
						function(event) // double click for max-win
						{
							windows.opens[$(this).closest('.window').attr('id')]
									.toggleSize();
							stopEvent(event);
				...
				.on(
						'click',
						'.close-win',
						function(event) // close-win
						{
							var q = $(this).closest('.window').attr('data-id');
							if (!q)
								q = $(this).closest('.app-btn').attr('data-id');
							if (!q)
								q = $('#taskMenu.show').data('id');
							windows.close(q);
							$('.categoryMenu li .app-btn')
									.each(
											function() {
				...
				.on(
						'click',
						'.min-win',
						function(event) // min-win
						{
							windows.opens[$(this).closest('.window').attr('id')]
									.hide();
							stopEvent(event);
						}).on('click', '.reload-win', function() // reload
																	// window
																	// content
				{
					var q = $(this).closest('.window').attr('data-id');
					...
					windows.openEntry(id);
						...
						'.delete-entry',
						function() {
							if (!$.isFunction(settings.onDeleteEntry))
								return;
							var id = $(this).closest('.dropdown-menu').data(
									'id');
							var et = getEntry(id);
							if (et
									&& confirm(settings.confirmRemoveEntry
											.format(et.name))) {
								settings.onDeleteEntry(et, function(result) {
									if (result) {
										$.zui.messager
												.info(settings.removedEntry
														.format(et.name));
										var option = {
											id : et.id
										};
					...
					if (!$.isFunction(settings.onUpdateEntryMenu))
						return;
					var id = $(this).closest('.dropdown-menu').data('id');
					var et = getEntry(id);
					if (et && (et.menu == 'all' || et.menu == 'menu')) {
						var menu = (et.menu == 'menu') ? 'none' : 'list';
						settings.onUpdateEntryMenu({
							id : id,
							menu : menu
						}, function(result) {
							if (result) {
	...
	windowsManager.prototype.handleWindowKeydown = function(event) {
		if (event.keyCode == 116 || (event.ctrlKey && event.keyCode == 82)) // F5
																			// or
																			// Ctrl+R
		{
			windows.reload();
			...
			try {
				window.frames[theLoadingFrame].event.keyCode = 0;
			} catch (e) {
			} // for IEs
	...
	windowx.prototype.init = function(et) {
		...
		if (this.$.length < 1)
			throw new Error('Can not find the window: ' + et.idstr
					+ ' when init it.');
	...
	windowx.prototype.reload = function(url, go2index) {
		if (!this.isLoading()) {
			...
			this.$.addClass('window-loading').removeClass('window-error').find(
					'.reload-win i').addClass('icon-spin');

			if (this.firstLoad)
				this.$.addClass('window-first');
	...
	windowx.prototype.loadIframe = function(go2index) {
		...
		if (frame) {
			try {
				if (go2index)
					frame.contentWindow.location.replace(this.indexUrl);
				else
					frame.contentWindow.location.replace(this.getUrl());
			} catch (e) {
				document.getElementById(fName).src = (go2index ? this.indexUrl
						: this.getUrl());
			}
		} else {
			this.$.find('.window-content')
					.html(settings.frameTplt.format(this));
		...
		frame.onload = frame.onreadystatechange = function() {
			...
			win.$.removeClass('window-loading').removeClass('window-first')
					.find('.reload-win i').removeClass('icon-spin');
			...
			try {
				...
				if ($frame.length) {
					...
					$frame.unbind('keydown', windows.handleWindowKeydown)
							.keydown(windows.handleWindowKeydown).data(
									'data-id', win.idStr);
				}
	...
	windowx.prototype.close = function() {
		var win = this.$;
		if (win.hasClass('window-safeclose')
				&& (!confirm(settings.confirmClose.format(win.find(
						'.window-head strong').text()))))
			return;
		...
		win.fadeOut(settings.animateSpeed, $.proxy(function() {
			$('.app-btn[data-id="' + this.id + '"]').removeClass('open')
					.removeClass('active');
			$('#s-task-' + this.id).remove();
			win.remove();
			if (this.isModal)
				$('#desktop').removeClass('modal-mode');
	...
	windowx.prototype.toggleSize = function(forceMax) {
		...
		if (forceMax) {
			...
			win.data('orginLoc', {
				...
				width : dSize.width,
				height : dSize.height
			}).find('.icon-resize-full').removeClass('icon-resize-full')
			...
			}).find('.icon-resize-small').removeClass('icon-resize-small')
	...
	windowx.prototype.afterResized = function(onlyAppSize) {
		if (!onlyAppSize) {
			if (this.isMax()) {
				this.$.width(desktop.width).height(desktop.height).css({
					left : desktop.x,
					right : desktop.y
				});
			}
		...
		var offset = this.$.hasClass('window-control-full') ? settings.windowHeadheight
				: 0;
	...
	function fullScreenApps() {
		...
		this.bindEvents = function() {
			$('.fullscreen-btn').click(function() {
				/*
				 * replace function 'show()' with 'toggle()' to change the
				 * behavoir
				 */
							...
							function(e) {
								...
								} else if (e.which == 13) // pressed enter
								{
									$selected.click();
										...
										|| e.which == 39 || e.which == 40) // pressed
																			// left,
																			// up,
																			// down,
																			// right
								{
									var $next = $selected.closest('li')[e.which > 38 ? 'nextAll'
											: 'prevAll']().not('.search-hide');
									if ($next.length) {
										$selected
												.removeClass('search-selected');
										$next.first().find('.app-btn')
								...
								if (val.length > 0) {
									...
									$('#allAppsList .app-btn')
											.each(
													function() {
														var btn = $(this);
														var r = true, et = getEntry(btn
																.attr('data-id'));
														var idkey = '#'
																+ et.id
																		.toLowerCase(), codeKey = et.code ? et.code
																.toLowerCase()
																: false, nameKey = et.name
																.toLowerCase();
														for ( var ki in keys) {
															var k = keys[ki];
															r = r
								...
								$('#appSearchNav > li.active').removeClass(
										'active');
								$(
										'#appSearchNav .app-search[data-key="'
												+ val + '"]').closest('li')
										.addClass('active').find(
												'.search-count').text(
												searchCount || '');
							});
		...
		this.handleHomeBlocks = function() {
			...
			function afterPanelRemoved(index) {
				if (settings.onDeleteBlock
						&& $.isFunction(settings.onDeleteBlock)) {
			...
			function afterOrdered(newOrders) {
				if (settings.onBlocksOrdered
						&& $.isFunction(settings.onBlocksOrdered)) {
		...
		this.show = function(id) {
			...
			$(
					'.fullscreen-btn[data-id="' + id + '"],.app-btn[data-id="'
							+ id + '"]').addClass('active');
			desktop.updateBrowserUrl();
			var et = getEntry(id);
		...
		this.toggle = function(id) {
			if ($('#' + id).hasClass('fullscreen-active'))
				this.hide(id);
			else
				this.show(id);
		};
		...
		this.afterBrowserResized = function() {
			this.$fullscreens.width(desktop.width).css({
				left : desktop.x,
				top : desktop.y,
				bottom : settings.bottomBarHeight
			});
			;
		};
	...
	function shortcuts() {
			...
			if (this.firstRender) {
				...
				this.$appsMenu.sortable({
					...
					always : function() {
						setTimeout(function() {
							$('.tooltip').remove();
							$('body').removeClass('mute-tooltip');
						}, 500);
					},
					finish : function(data) {
						if (!$.isFunction(settings.onSortEntries))
							return;
						...
						data.list
								.each(function() {
									orders[$(this).find('.app-btn').data('id')
											+ ''] = orderNext++;
								});
		...
		this.showAll = function() {
			...
			if (desktop && desktop.isFullscreenMode
					&& $('#search').val() !== '') {
		...
		this.show = function(et) {
			var $shortcut;
			var activedId = (windows && windows.shows.length) ? windows.shows[windows.shows.length - 1]
					: null;
			if (et.hasMenu) {
				$shortcut = $(et.toLeftBarShortcutHtml());
				if (et.code == '') {
					$shortcut.find('button').attr('id', 'category' + et.id)
				...
				if (et.category !== undefined && et.category != 0) {
					if ($('#categoryMenu' + et.category).length == 0) {
						$('#leftBar #apps-menu .bar-menu:first').after(
								$('#categoryTpl').html().replace(/categoryid/g,
										et.category));
					}
		...
		this.bindEvents = function() {
			$(document).on('click', '.window-btn', function(event) {
				...
				return false;
					...
					'.app-btn',
					function(event) {
						if ($(this).hasClass('categoryButton'))
							return false;
						var $this = $(this);
						var et = getEntry($this.attr('data-id'));
						if (et) {
							if (et.display == 'fullscreen'
									&& desktop.fullScreenApps)
								desktop.fullScreenApps.toggle(et.id);
							else
								windows.openEntry(et, $this.data('url')
										|| $this.attr('href'));
						} else {
							bootbox.alert(settings.entryNotFindTip);
						}
			...
			$(document)
					.on(
							...
							function(e) {
								if (e.which == 3) {
									var btn = $(this), menu = $('#taskMenu');
									if (!menu.length)
										menu = $(settings.taskBarMenuTplt)
												.appendTo('#desktop');
									...
									var isListBtn = btnType === 'list', isTaskBtn = btnType === 'task';

									menu.find('.reload-win').toggle(
											!isListBtn && et.opened);
									menu.find('.fix-entry').toggle(
											!isTaskBtn && !et.hasMenu);
									menu
											.find('.remove-entry')
											.toggle(
													!isTaskBtn && et.hasMenu
											...
											.find('span')
											.text(
													settings[isListBtn ? 'removeFromMenuText'
															: 'removeText']);
									menu.find('.close-win').toggle(
											!isListBtn && et.opened);
									menu.find('.delete-entry').toggle(
											superadmin && !et.sys && isListBtn);
									...
									if (btnType == 'menu') {
										...
										menu
												.css({
													left : e.clientX - 2,
													top : (menuHeight
															+ e.clientY > desktop.height) ? (e.clientY
															- menuHeight - 2)
															: e.clientY,
													bottom : 'inherit'
												});
										...
										menu
												.css({
													left : offset.left,
													top : 'inherit',
													bottom : settings.bottomBarHeight + 2
												});
										desktop.menu.hideMoreMenu();
									}

									desktop
											.toggleDropmenuMode('taskmenu',
													true);
	...
	function menu() {
		...
		this.hide = function() {
			...
			setTimeout(function() {
				if (!$leftBar.hasClass('menu-show')) {
					...
					$('#apps-menu .app-btn[data-toggle="tooltip"]').removeAttr(
							'data-toggle');
				}
			}, 1000);
		};
		...
		/* Show the menu */
		this.show = function() {
			desktop.menu.$leftBar.removeClass('menu-hide')
					.addClass('menu-show');
			setTimeout(function() {
				$('#apps-menu .app-btn').attr('data-toggle', 'tooltip');
			}, 500);
		};
		...
		this.showMoreMenu = function() {
			...
			if (desktop.toggleDropmenuMode)
				desktop.toggleDropmenuMode('more-option', true);
		};
		...
		this.hideMoreMenu = function() {
			...
			if ($menu.hasClass('show')) {
				...
				if (desktop.toggleDropmenuMode)
					desktop.toggleDropmenuMode('more-option', false);
			}
		};
